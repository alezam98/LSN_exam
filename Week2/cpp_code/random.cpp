...
// estrazione da pdf lineare
double Random :: sample_pdf() {		
   double x=Rannyu();
   return 1 - sqrt(1 - x);	//p(y) = 2(1 - y)
}
...
vector<double> Random :: sample_pdf(int n) {		
  vector <double> v;
  for(int i=0; i<n; i++)
  	v.push_back( sample_pdf() );
  return v;
}



// estrazione di un versore (da pdf sferica uniforme)
double* Random :: versor(){
