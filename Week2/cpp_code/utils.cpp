...
// pdf da usare per importance sampling (vedi random.cpp, "sample_pdf")
double pdf(double x){
	return 2.*(1. - x);
}

// funzione per calcolo integrale con metodo della media
...
// funzione per calcolo integrale con metodo dell'importance sampling	
double integrale_sample (int n, Random & rnd){
	...
	for(int i=0; i<n; i++){
		x = rnd.sample_pdf();
		I += integranda(x)/pdf(x);
	}

	return I/n;
}

// funzione per calcolo integrale con metodo accept-reject
