...
# parametri generali
sed -i "1s/.*/1/" input.in             # simul
sed -i "17s/.*/single_step/" input.in  # dir
sed -i "9s/.*/100000/" input.in        # nblk
sed -i "10s/.*/1/" input.in            # nstep
sed -i "2s/.*/0/" input.in             # verbose
sed -i "13s/.*/0.05/" input.in         # prec
...
do
    ...
    then
        ...
        sed -i "8s/.*/0.12/" input.in  # delta
        echo "-- $state: T=0.8, rho=1.1, rcut=2.2, delta=0.12"
    elif [[ $state == "liquid" ]]
    then
        sed -i "4s/.*/1.1/" input.in   # T
        sed -i "6s/.*/0.8/" input.in   # rho
        sed -i "7s/.*/2.5/" input.in   # rcut
        sed -i "8s/.*/0.2/" input.in   # delta
        echo "-- $state: T=1.1, rho=0.8, rcut=2.5, delta=0.2"
    else
        sed -i "4s/.*/1.2/" input.in   # T
        sed -i "6s/.*/0.05/" input.in  # rho
        sed -i "7s/.*/5.0/" input.in   # rcut
        sed -i "8s/.*/0.2/" input.in   # delta
        echo "-- $state: T=1.2, rho=0.05, rcut=5.0, delta=0.2"
    fi
